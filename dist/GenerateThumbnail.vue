        ...
        <v-tabs-items v-model="tab">
            <v-tab-item style="height: 700px">
                <v-layout row="" wrap="" pa-2="" style="height: 600px; overflow: scroll">
                    <v-flex md6="">
                        <video ref="video" :src="vSource" style="width: 100%; height: auto" type="video/mp4" controls="" crossorigin="anonymous"></video>
                    </v-flex>
                    <v-flex md6="" pl-1="">
...
var _default = {
  ...
  computed: {
    vSource() {
      return _cms.default.baseUrl + `video/${this.source}`;
    }
  ...
  methods: {
    ...
    async save() {
      ...
      const Content = _cms.default.getModel('Content');

      await Content.findByIdAndUpdate(this.id, {
    ...
    getBase64FromSource(src) {
      return new Promise(resolve => {
        ...
        this.drawImg(canvas, src).then(cv => {
    ...
    upload() {
      ...
      this.generateThumbnailFromFile(file).then(async base64 => {
        const Content = _cms.default.getModel('Content');
