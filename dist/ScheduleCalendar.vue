<template>
    <div>
        ...
        <full-calendar :config="config" :events="events" ref="calendar" @event-created="select" @event-resize="onResized" @event-drop="onDrop">
    </full-calendar></div>
</template>
<script>
"use strict";
...
var _default = {
  ...
  data() {
    return {
      ...
      config: {
        ...
        columnHeader: true,
        columnFormat: 'ddd',
        timezone: 'local' // eventRender: function (event, element) {
        //   console.log(event);
        // },
        // eventDragStart: e => {
  ...
  methods: {
    ...
    push() {
      this.events.push({
        title: 'another test',
        start: (0, _moment.default)().add(4, 'h'),
        end: (0, _moment.default)().add(5, 'h')
      });
    },
      ...
      for (let i = 0; i < schedule.length; i++) {
        schedule[i].weekdays.forEach(item => {
          events.push({
            ...
            index: events.length,
            start: (0, _moment.default)(`${item}-${schedule[i].start}`, 'ddd-HH:mm'),
            end: schedule[i].in === 'next day' ? (0, _moment.default)(`${item}-${schedule[i].start}`, 'ddd-HH:mm').add(1, 'days') : (0, _moment.default)(`${item}-${schedule[i].start}`, 'ddd-HH:mm')
          });
        });
      }
    ...
    getDaysBetweenDates(start, end) {
      ...
      function getNextDay(current, end) {
        ...
        if (!(0, _moment.default)(e).add(1, 'day').isSame((0, _moment.default)(end), 'day')) {
